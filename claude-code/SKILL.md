...
## Launching with `--remote-control`

`--remote-control` lets you start a Claude Code session that can be driven from claude.ai or the mobile app. This is handy for kicking off a long-running interactive session you want to monitor or steer from elsewhere.
...
Add `--dangerously-skip-permissions` if you want the session to auto-approve tool use (use with care — see the Permission & Safety flags above).

**Choosing the working directory:** always start Claude Code in the project you want it to operate on. Either `cd` into the directory first, or rely on the directory you launch from. Keep one session per project for clean, focused context.

### Backgrounded / programmatic launch (PTY required)

To launch a remote-control session in the background (so it keeps running while you do other things), it must run under a PTY — Claude Code is a full TUI and exits immediately without one. With most terminal-automation tooling that means enabling both background and PTY modes:

```python
terminal(
    background=True,
    pty=True,
    command="cd /path/to/project && claude --dangerously-skip-permissions --remote-control my-session",
    watch_patterns=["remote control", "Remote Control", "/rc active", "active"],
)
```

**Why PTY is required:** without it, Claude Code exits with `Error: Input must be provided either through stdin or as a prompt argument when using --print`. `background=True` + `pty=True` together keep the interactive TUI alive.

**Success indicator:** the session's output/status bar will show `/rc active`.

### Verifying `/rc active` after launch

After launching, confirm `/rc active` appears in the status bar. If it shows `Remote Control failed · /login` instead, you need to re-authenticate (see below) before the session is usable. Don't report success without this check.

### Re-authenticating (`/login`)

If the status bar shows `Remote Control failed · /login`, or auth has otherwise expired, run `/login` inside the session (interactive mode) to surface the OAuth flow:

1. Type `/login` and press Enter in the session.
2. Pick a login method:
   - **Claude subscription** (Pro/Max/Team/Enterprise) — default; opens a browser OAuth flow.
   - **Anthropic Console** — API-key billing.
   - **3rd-party platform** — Amazon Bedrock, Microsoft Foundry, or Google Vertex AI.
3. For subscription OAuth, open the printed `https://…` authorize URL in a browser and approve. The session completes the callback automatically.
4. Verify with `claude auth status` and re-confirm `/rc active`.

**Vertex AI:** select 3rd-party platform → Google Vertex AI → Application Default Credentials, then enter your GCP Project ID and region.

**Tip:** if you're driving the session remotely (not at the machine), copy the OAuth URL out and approve it from any browser — including a phone — then let the session finish the callback on its own.


## Pitfalls & Gotchas

1. **`claude -p` in terminal() has a 60s default timeout** — for GUI automation tasks that write a script and run it, set `timeout=120` or higher, or better yet: write the script yourself then run it directly with `/usr/bin/python3 /tmp/script.py` — no need to delegate to claude CLI for simple execution tasks.
...
2. **`--remote-control` requires `background=True` + `pty=True`** — without `pty=True`, Claude Code exits immediately with stdin error. `script -q /dev/null` wrapper produces no output. Both flags are required.
...
13. **Terminal.app `make new tab` AppleScript is broken** — `-10000` error every time. The only reliable way to open a tab is `Cmd+T` via System Events keystroke, then `do script "..." ` with NO `in tab` argument (it targets the active/new tab). Never use `make new tab` or `do script ... in tab N of w` for newly created tabs.
